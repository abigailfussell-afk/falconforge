    ...
    return (
        <div className="flex flex-col md:flex-row gap-2.5 h-full md:pb-2 overflow-y-auto md:overflow-x-auto">
            {STATUS_COLUMNS.map(status => {
                ...
                return (
                    /*
                     * THE FIFTH COLUMN HAS TO BE ON SCREEN (WALK-A-05).
                     *
                     * `md:w-64` was a fixed 16rem per column: five of those plus four gaps is
                     * 1320px, and a 1280px laptop with the 14rem sidebar has roughly 1024px of
                     * main area — so Done sat off the right edge behind a horizontal scrollbar,
                     * on the most common desktop size there is. A kanban board whose last column
                     * is the one you cannot see is a board that does not answer "is this sprint
                     * finishing".
                     *
                     * So the columns SHARE the width rather than claiming it: `basis-0` with
                     * `flex-1` gives five equal shares of whatever is actually there, and
                     * `min-w-44` (11rem) is the floor below which a card stops being readable —
                     * at which point the container's `overflow-x-auto` takes over, as it still
                     * must on a tablet. Collapsed columns keep their fixed 11 and let the
                     * others expand into the space.
                     */
                    <div
                        key={status}
                        className={`bg-slate-100 dark:bg-slate-800 rounded-xl flex flex-col shrink-0 transition-all ${isCollapsed ? 'md:w-11 h-auto' : 'md:flex-1 md:basis-0 md:min-w-44 md:h-full'}`}
                    >
                        <button
