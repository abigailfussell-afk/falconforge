...
| 2026-09-14 | **Hotfix — the invite panel could not create an invite (SEC-17 regression)** | `v2/hotfix-invite-panel` | **Complete, applied to production and merged.** Reported by Kevin from team #30727. SEC-17 (`20260828000000`) made `invites.code DEFAULT generate_invite_code()`, a SECURITY DEFINER function with EXECUTE revoked from every API role, on the belief that a DEFAULT needs no privilege; it runs as the inserting role and EXECUTE is checked. Every "Generate Link" since 2026-08-28 returned 42501, and production held **zero panel-created invites in 16 days**; `create_team_as_admin` kept working because it runs as its owner, which is why nothing looked wrong. Fix `20260914000000`: SECURITY INVOKER + EXECUTE for `authenticated` only; `invites.code` INSERT/UPDATE stay revoked (verified on the hosted project). **Why the suite missed it**: every successful insert in the SEC-17 file ran as `service_role`, and the one admin-client test expected a refusal and accepted ANY permission error — so it was green over the bug, and the cross-tenant invite INSERT cases were being refused by the function, not by `invites_insert_roster`, for 16 days (`docs/failure-modes.md` §2 and §3 at once). New tests insert exactly as `InviteManager` does and require a student's refusal to be RLS's; both watched red with the production error. `gate:db` green (1241 unit / 99 integration / build / schema assertions / 674 db / 418 rls); `probe-invite-panel.mjs` clicks the real button, 5/5 fixed and 1/5 with production's state restored locally. Order per `docs/beta-ops.md`: dump, dry run, `db push --linked`, privilege check, then merge. |
| 2026-09-13 | **Sprint 37 — BIOBUZZ, the day after the reveal (R-02)** | `v2/sprint-37-biobuzz` (off `v2/kickoff-readiness-fixes`, which is not yet on `main`) | **Complete except the release, the image licence, and a production check — all Kevin's.** Gate green (lint / 1241 unit +2 skipped / 99 integration / build), e2e **40/40** across both projects, two browser probes green on a `build:local` at 375 px (`probe-biobuzz` 22/22, `probe-events-and-form` 19/19). `supabase/` untouched. **The definition** is written from Competition Manual V1 Table 10-2, one field per scored action a scout can attribute to one robot: LEAVE, PARK in AUTO and at the end, launches into the CELL and HIVE TIPS in both periods, misses, GARDEN, NECTAR and POLLEN in a FLOWER in the final minute, two pickup capabilities, rating, notes. The placeholder's "End Game" phase is gone because BIOBUZZ has none — AUTO 30 s, an 8 s transition, TELEOP 2:00, FLOWERS unlocking at 1:00. Six summary metrics, TIPS among them because the POLLINATOR RPs turn on 4 and 7. **The field** is `BiobuzzField.webp`, 86 KB, 1024², a crop of Figure 9-2 (an orthographic render of the official CAD) with the magenta callouts painted out; precached, and loaded with the network OFF in the probe. The first pass of the paint-out matched pink highlights on the FLOWERS and the red HIVE and smeared them — found by looking at the image, fixed by confining the fill to the callouts. **No component changed: the app was already data-driven (Sprint 18), and that held.** What did NOT hold is everything outside `src/` that named a game, none of which the Gate runs: `e2e/scouting-summary.spec.ts` typed into `field-autoScore` (placeholder-only) and `e2e/team-lifecycle.spec.ts` into DECODE's `field-endGameNotes` — both watched failing, both now read the field from `CURRENT_GAME`; **the second was missed by this sprint's own grep** and found only by running the whole pack, so the lesson is the pack, not the grep. `games.ts`'s JSON imports lacked `with { type: 'json' }`, so the first spec to import it could not load — the exact trap `attestation-versions.ts` documents. Both seeds labelled 2026-27 DECODE (now BIOBUZZ, id and version read from the JSON), and `probe-events-and-form` drove DECODE fields and waited for a team picker the kickoff pass removed for one-team accounts. The card showed "Auto: launched into Cell" beside a bare "Launched into Cell" — unambiguous in the form under its section heading, ambiguous on a card that has none; every phase-ambiguous label now names its phase. Four guards, each watched red: BIOBUZZ's key set pinned; no duplicate keys (PARK scores twice, so it is the field a hand-written definition keys twice); every metric reads a numeric field (a typo is a column of "—" all season, silently); every game has its own field image and the file exists — which is R-02 itself. **The probe's own first run lied**: it queried `team_number` (the column is `opponent_team_number`), read the 400 as "not synced yet" and reported the report missing; it throws on a query error now. Five parking-lot entries. |
| 2026-09-05 | **Kickoff readiness fixes — R-01 and the six small ones** | `v2/kickoff-readiness-fixes` | **Complete except the real-device half, which is stated rather than skipped.** Gate green (lint / 1235 unit +2 skipped / 99 integration / build). Work list from `docs/kickoff-readiness-review-2026-09-05.md`. **R-01 is the one that mattered: any tab hidden→visible transition unmounted the whole app and destroyed every open form.** `auth-js` re-emits `SIGNED_IN` from `_recoverAndRefresh` on every `visibilitychange`, `auth.tsx` treated every `SIGNED_IN` as a reason to hold `isLoading`, and `App.tsx:204` renders the splash INSTEAD of `<Routes>` while loading. At a venue that is a half-filled scouting report lost when the scout glances at the camera or the phone locks. A ref now records the user id the profile sync has run for, and a re-emission for that id updates the session and nothing else — no splash, no unmount, and no second profile REST call per refocus. **The ref rather than a look at `state.user`, and that is load-bearing**: the mount's `getSession()` records the user WITHOUT syncing while `INITIAL_SESSION` is the event that must sync, so comparing against state would make the boot event look like a re-emission whenever `getSession()` won the race and hang on "Preparing your workspace..." forever. Five regression tests; three go red with the guard removed and two more go red if the guard is made too WIDE (a different user on a shared laptop, and signing back in after a sign-out). **The unmount test was decoration until it was fixed**: dispatching the event and flushing the deferred sync inside ONE `act` lets React coalesce the two `isLoading` writes into a single render, so the splash never commits and it passed against the unfixed code — split into two `act` calls it goes red. **And two of the five flaked under full-suite load, passing alone** (`npm run test:run` red, the file green five times running): they waited a fixed macrotask for a profile sync that settles later than that under load. The two that assert a HELD splash now freeze the profile read instead of racing it, because a transient asserted against a loaded runner is not an assertion. **R-04, measured rather than eyeballed** (`docs/failure-modes.md` §5): at 1280px in a real browser the old board's container was `scrollWidth 1320` against `clientWidth 1024` and the Done column's right edge sat **296px past** the container — five fixed `md:w-64` columns plus gaps is 1320px and a 1280 laptop has ~1024px of main area. Columns now share the width (`basis-0` + `flex-1`) to an 11rem floor: 1024/1024, no overflow, Done ends exactly at the edge, each column 197px. At 768px they hit the floor at 176px and the container scrolls (920 vs 736), which is the tablet case and unchanged in kind; at 375px it still stacks. **R-05**: the scouting form opened with "Enter a team number" already in red under an untouched box. Only the EMPTY case waits for a touch — "digits only" and the length cap are about something typed and stay immediate — and nothing about saving changed. **R-06**: the empty-team greeting told a STUDENT to "plan your first sprint below", an instruction about a team they did not set up pointing at a control the roster does not give them. `isMentorOrAbove`, the predicate the shell and Training already ask. **R-07 (Kevin: 14 days)**: the team-facing expiry warning was 30 days and a new team's probation grant is exactly 30 days (D3), so "your licence ends in 30 days — after that the team becomes read-only" was the first line of every screen from the minute a coach finished registering, on the healthy path. Split into `TEAM_EXPIRY_WARNING_DAYS` (14) and `OPERATOR_EXPIRY_WARNING_DAYS` (30): one constant was answering two different questions and shortening the operator's own horizon was not part of the decision. **R-08**: the landing page named three of the five people this product has — `admin` was folded into the coach card as a badge, and guardians, who hold the login for every member under 13, were absent entirely. Five cards, verified rendering in a production build. **R-09**: a user with one team met "Select a team to continue" listing one team. The guards are the part worth reading — "Create a team" lives on that screen and nowhere else, so an unconditional skip would make a SECOND team unreachable (a school with two FTC numbers, which Kevin flagged); `askedForThePicker` already answers it, since the sidebar's "Switch team" is rendered for every signed-in user however many teams they have. A pending join request and a stored invite code also hold the picker, because both are said there and nowhere else. **`operator_grant_extra_team` finally has a button** — it shipped with the D3 gate and nothing called it, so the first multi-team coach was a support email answered with hand-written SQL. Offered under the same "this row is a real account" rule as Erase, because a child profile's `user_id` is the GUARDIAN's. **Kevin's decisions this pass**: R-03's attestation copy ships AS IS (beta coaches attest to a per-member billing model that may never exist — no version bump, no re-attestation); the "Draft — pending legal review" banner is DROPPED, the 2026-08-22 review judged to cover the documents, and the legal test now asserts the inverse so it cannot grow back; the `token_hash` link-scanner route is NOT built this pass. **NOT VERIFIED, and it is the important gap**: R-01 has not been exercised in a running app, because Docker was down and every screen it affects is behind auth. The test evidence is jsdom-shaped and jsdom never fires `visibilitychange` — which is exactly how this defect survived 1,200 tests. It needs the lock/unlock run on a real phone from §5 of the review before kickoff. **One data point for the exit-127 item**: the pre-commit hook died once with lint-staged's "Task failed to spawn: vitest related --run", not a test failure; the identical command run by hand immediately afterwards passed (29 files, 339 tests) and the retried commit was clean. Same signature as the Gate death — a spawn that does not happen, at a random moment. |
| 2026-08-29 | **Sprint 36 — the fixture leak fixed, and the exit-127 death that would not reproduce** | `v2/sprint-36-gate-reliability` | **Partially complete, and the incomplete half is the headline.** `gate:db` green (1213 unit / 99 integration / 672 db / 418 rls). **THE EXIT-127 DEATH DID NOT REPRODUCE** — four consecutive clean `gate:db` runs today — so it is not fixed and is not claimed to be. **The documented lead is also wrong on its face**: it blames `globalSetup`'s `shell: true` spawn on the strength of a `DEP0190` warning, but globalSetup runs ONCE at startup and cannot explain a death "at a different suite each time". Recorded so the next person does not spend a day there. **What WAS fixed** is the fixture leak it was blamed for: Sprint 29 swept leftover TEAMS and nothing swept USERS, which leak through the same hole. **And they leak from SUCCESSFUL runs too** — `cleanup()` calls `deleteUser` without checking the result, so it fails silently on anything still referenced; 11 were left by two clean runs. Unlike teams they collide with nothing, so they cause no failures and simply accumulate until something else trips over them, which today was `npm run seed:review` refusing over 11 accounts it had never created. Both halves fixed: the sweep skips and COUNTS users pinned by `operator_actions` (NO ACTION by design — an audit entry must name who made it) rather than dying on the first, and the seed's assertion now reads the accounts it created. **Proved by planting**: 14 leftovers, one pinned, 2 swept, 12 remaining. **And the sweep's first version reported a plausible lie** — written as one statement, the `count(remaining)` beside a `DELETE` sees the pre-delete snapshot, so it said 14 after sweeping 2 of 14; every number was a real number about something, so only arithmetic caught it. The seed fix was falsified both ways: it succeeds with the 12 leftovers that made it refuse, and still fires "36 of 36 accounts seeded by THIS RUN" when `privacy_accepted` is dropped from `makeUser`. **One more finding**: there is ONE local database and no guard against concurrent use — an experiment run beside a live `gate:db` measured that run's own fixtures as leftovers and had to be discarded. |
| 2026-08-29 | **Sprint 35 — the email address, and a screen that must not say "Saved"** | `v2/sprint-35-email-change` | **Complete.** Gate green; verified in a browser at 393px. `supabase.auth.updateUser` was called in exactly two places and neither touched `email`, so nobody could change their address without asking Kevin. **The parking lot's reading was exactly right — a missing SCREEN, not a missing capability**: `profile-mirror.db.test.ts` already proved the server carries an email change into `public.users`, and SEC-16 carries it onto every child's roster row, which is what makes this a guardian problem first — a managed child has no login, so the guardian's address is the only contactable one the team holds for them. **THE COPY IS THE REVIEWABLE PART.** GoTrue swaps the address only when the emailed link is followed, so "Saved" would be a lie told at the exact moment a typo is still recoverable. The message names the destination *and* states the account still uses the old address until confirmed; the test asserts both appear and that "saved" does not, and it goes red against `'Saved! Your email address has been updated.'`, written deliberately to check. Local state is NOT updated on success, for the same reason. The shape check is deliberately LOOSE, because a strict regex rejects valid addresses and the failure mode is a coach who cannot enter their own school address; the same-address check is case-insensitive, because GoTrue would otherwise email a confirmation for a change to nothing. Offline the control is disabled WITH a reason — the one control on that screen that cannot be queued. **One parking-lot entry, and it is a gap in Sprint 29's own fix**: the fixture sweep clears leftover TEAMS and not leftover USERS, discovered by `seed:review` refusing to run over 11 uuid-shaped fixture accounts it did not create — two faults meeting, since the seed also asserts over every row in `auth.users` rather than the 36 it owns. |
| 2026-08-29 | **Sprint 34 — an event has an identity, not only a spelling** | `v2/sprint-34-scouting-events` | **Complete, migration applied to production first, then merged and deployed.** `gate:db` green (672 db tests including 5 new, 418 rls). `scouting_reports.event_name` was free text and the summary grouped on it EXACTLY — a case-sensitive Map — so two scouts at one competition typing "League Meet 1" and "League meet 1" produced two events and two summaries with nothing to say so. Scouting silently cut in half is worse than scouting that is missing, because the numbers still look plausible. **THE FK IS COMPOSITE AND THAT IS THE LOAD-BEARING PART**: `(season_event_id, team_id)` against `competition_events (id, team_id)`, because a plain reference to `id` would let one team's report point at another team's event — B21's shape, where knowing a uuid is the whole attack. Proved by swapping the local constraint to single-column: the cross-tenant test fails and NOTHING ELSE does. `ON DELETE SET NULL (season_event_id)`, the per-column form, because the whole-key form cannot fire when `team_id` is NOT NULL — the trap this schema already carries on five constraints. **Two halves, neither sufficient alone**: normalising the label fixes the accidental split, and the picker fixes the deliberate one, since "MI State" and "Michigan State Championship" are different strings by any measure. Free text stays PERMANENT — a scout at an event the coach has not entered must still record what they saw. **Two defects `tsc` accepted**: the filter still compared raw `eventName` while the dropdown emitted group keys, and the CSV filename would have carried `name:league meet 1` into a file a lead keeps. Production preconditions checked before applying (0 reports, 0 events, composite target present); the migration was the first in this project applied behind a backup PROVEN restorable the same day. |
| 2026-08-29 | **Sprint 33 — the red one, and the first beta blocker** | `v2/sprint-33-bundle-target` | **Complete.** Gate green. **The parking lot's only remaining 🔴 is closed**: `dist/` was ambiguous about which backend it targeted, and `npm run gate` leaves a PRODUCTION-wired bundle there every time it runs (its final `vite build` has no `--mode`, so it reads `.env.local`). `npm run check:bundle` answers the question statically — no server, no browser, no Docker — so it can run before anything expensive or irreversible, and `npm run preview:local` builds, checks and serves in one step. **It asserts the local stack is PRESENT rather than production being absent**, because a stale or broken build references neither and would pass the negative form (§4). All three states exercised. `environment-divergences.md` §8 rewritten — it blamed `test:e2e`, and the Gate does this far more often. **Beta blocker: the car-park case.** `create_team_as_admin` has always returned `season_id`; `CreateTeam` seeded the TEAM from the write, under a comment explaining why that matters, and threw the SEASON away — so `currentSeasonId` stayed null until the pull landed, and offline it never lands. A coach who registers and walks into a venue with no signal got a read-only board explaining itself with "Select a season first" about a season never offered. Seeded from the write now. Three tests, two watched failing with `expected null to be 's1'`; the third passes both ways ON PURPOSE, guarding against a future version seeding a blank season to make the others green. **The `as any` ratchet caught this sprint's own tests** — three casts copied from the surrounding file, which predates the ratchet and carries a typed `rpcMock()` helper whose comment says exactly that. Principle 7 held; the guard did the work that reading the file should have. **One change is NOT verified locally**: the `offline-sync.spec.ts` workaround removal, since Docker was down — CI is the check, and it is the one commit to revert if wrong. |
| 2026-08-25 | **Sprint 32 — the scroll that reported itself working** | `v2/sprint-32-fieldset-scroll` | **Complete.** Gate green; new probe 6/6. Sprint 31 fixed the modal's *appearance* — the footer is opaque and the see-through is gone — and Kevin reported the same modal still could not be scrolled, with a screenshot taken at the furthest point it would travel. **A `<fieldset>` is not a scroll container in Chromium.** It lays its children out in an anonymous box and refuses to scroll, and it still REPORTS an overflowing `scrollHeight`. Measured on a Pixel 5 profile: clientHeight 480, scrollHeight 765, and `scrollTop = 99999` left `scrollTop` at **0**, with the Description textarea at y 546–674 inside a container ending at 605 — permanently unreachable. `overflow-y-auto` moved to a wrapping div; the fieldset keeps `disabled`, which is the one thing it is genuinely good at. After: `scrollTop` reaches 285. **THE PART WORTH KEEPING IS WHY IT SURVIVED TWO PASSES.** Both earlier checks asserted `scrollHeight > clientHeight` and reported "canScroll: true" — a property that says the CONTENT overflows and says nothing about whether the ELEMENT scrolls, and which is true of the broken version. That is `docs/failure-modes.md` §2 exactly, written twice, by an agent narrating that same pattern to Kevin in the same session. The new probe sets `scrollTop` and asserts it MOVED, and its header says why it must not be "simplified" back to the cheaper property. Two guards at two costs: `scripts/probe-modal-scroll.mjs` (3/6 against the fieldset, needs Docker) and a harness ratchet refusing `overflow-*` on any `<fieldset>` (runs in the Gate, everywhere). Both watched failing. |
| 2026-08-25 | **Sprint 31 — three more from a phone, and one that was geometrically perfect** | `v2/sprint-31-modal-scroll` | **Complete.** Gate green (`GATE_EXIT=0`), accessibility probe 12/12. **Sprint 30's `dvh` fix WORKED** — the task modal's footer is visible where it used to be cut off — and that revealed two things under it. **The screenshot gave away the first: the title input was focused, so the keyboard was up.** `index.html` had no `interactive-widget`, and Android Chrome's default is `resizes-visual`: the keyboard shrinks the VISUAL viewport but not the LAYOUT viewport, and `dvh` follows the layout one — so an `85dvh` modal stayed full height while the keyboard covered the bottom third. `interactive-widget=resizes-content` now. The second was **50% alpha on bands the body scrolls under**, which is why placeholder text was readable through the buttons; opaque `dark:bg-slate-700` plus `shrink-0`. **I had looked at those alpha bands the day before and deliberately left them**, judging them a legitimate recessed treatment — right for a header on a panel, wrong for a footer content passes beneath, and only a screenshot settled it. **A hypothesis checked and discarded**: the body was NOT missing `min-h-0` — `min-height: auto` does not apply to a flex item that is a scroll container, and it held 735px in 406px and scrolled correctly at 375×640. **The dashboard tile**: label at 60px, value at 25px — the number aligned to the ICON rather than to its own label, because it was a sibling of the icon+label row rather than sharing its column. Now both 60px. **AND THE ONE WORTH READING: the sidebar profile row was geometrically PERFECT.** `avatarCentre − columnCentre` measured exactly **0**, and it still looked uncentred — Sprint 19's touch floor puts `min-height: 32px` on every `a[href]` under `pointer: coarse`, so "Edit Profile" was a 32px box around 11px of text stacked under a 16px name, and the two lines drift apart inside boxes that are correctly centred. Exempting the link would have shrunk a real tap target to fix an appearance; the name and link are ONE control now, so the floor stands and the person's name became tappable. **Verification honesty: the `interactive-widget` half cannot be checked here** — headless Chromium has no soft keyboard — so it joins `dvh` as a fix whose mechanism is certain and whose sufficiency is unobserved. The opaque footer removes the visual symptom either way. **Also caught in this sprint: Sprint 30 never got a §8 row at all** — its parking-lot entries were written and the progress log was skipped, which CLAUDE.md requires each sprint. Added here alongside this one. |
| 2026-08-24 | **Sprint 30 — three mobile defects, and the environment that hid two of them** | `v2/sprint-30-mobile-modals` | **Complete, deployed.** Gate green; CI and Deploy green; verified in the served production CSS. Three defects reported from a phone. **The Competitions add-event row was the roster bug a THIRD time**: the name input `min-w-0 flex-1` beside a `w-32 shrink-0` code field and a rigid native date picker, inside a `flex-wrap` that can never fire because `flex-1` is `flex: 1 1 0%` and its hypothetical size is zero — measured at 375px, the one REQUIRED field rendered ~60px wide and showed "Ev". **The import modal's "fade through"** was a band painted `dark:bg-slate-900` at full opacity — measured header `rgb(15,23,42)` against page `rgb(15,23,42)`, IDENTICAL, with only a `bg-black/50` overlay between them, so the page read straight through it. Fixed in four places. **The task modal's missing footer was `85vh`**: `vh` resolves against the LARGE viewport, the one with the URL bar hidden, so on a phone showing its bar the modal is sized against a viewport taller than the visible area and the Save/Cancel row lands under the browser chrome with `overflow-hidden` and nothing to scroll. The token's own comment claimed it "leaves room for … a phone's URL bar" — describing `dvh` while the value said `vh`. **Two of the three are invisible to every check here**, so both carry SOURCE-LEVEL ratchets rather than measurements, and the colour ratchet caught a footer I had missed on its first run. **My own `line-clamp-2` fix did nothing at first** — written as `block line-clamp-2`, and Tailwind emits `.block` later in the stylesheet, so `display:block` cancelled the clamp and it rendered three unclamped lines that looked plausible; caught by counting rendered lines. **One 🔴 parking-lot entry, and it is mine**: `npm run gate` ends in a default-mode `vite build`, which loads `.env.local` — production — and silently repoints `dist/`, so any `vite preview` afterwards serves a bundle wired to the real project. A login attempt reached production's GoTrue before the cause was found. |
| 2026-08-24 | **Sprint 29 — the quick wins, the Gate's reliability, and the bug Kevin found on a phone** | `v2/sprint-29-quick-wins` | **Complete.** Gate green (`GATE_EXIT=0`, precache 1,582 KiB); full db suite **32 files / 667 tests** green in one run, which is also the first `test:db` run in a while not to die part-way — recorded as one clean run, NOT as evidence the exit-127 item is closed. **THE ROSTER ROW WAS THE REAL FIND, and it came from Kevin looking at his phone rather than from any suite.** Admin Settings at 375px rendered ONE CHARACTER of a 25-character name: the row 293px, the controls beside it 249px, the name column **32px**, the name itself an 18px box holding 204px of text, and the 32px avatar **3px** across. The markup already looked like it handled this — the row is `flex-wrap` and the text column is `min-w-0`, both the right instinct, and together they are the bug: the controls are a flex container with the default `min-width: auto` so they cannot shrink, and `flex-1` is `flex: 1 1 0%` so the name column's HYPOTHETICAL size is zero, the two children therefore always "fit" on one line, the wrap never fires, and the name column absorbs the whole deficit alone. `basis-full sm:basis-0` plus `shrink-0` on the avatar; desktop **measured** unchanged at 713–739px, not assumed, because Sprint 19's touch-target floor leaked onto the desktop in exactly this way. New `scripts/probe-roster-row.mjs`, because jsdom renders the broken and fixed versions identically (§5) — watched failing at 4/7 with the real numbers in its own failure text. **It deliberately does not rest on "no horizontal overflow", which PASSED against the defect**: nothing overflowed precisely because the column collapsed instead, which is §2, a check satisfied by a state the defect also produces. **Five one-line fixes, each watched failing.** `"[object Object]"` (two entries, one line): three of five new tests go red on the old code with `expected '[object Object]' not to be '[object Object]'` — **and the one that stays GREEN is the lesson**, since a test throwing `new Error('boom')` passes either way and that is how this survived unnoticed since Sprint 11. The **ninth** initials implementation deleted (`MJ` where `member-utils` says `MW`), from a file that already imported the canonical helper. `emailRedirectTo` on signup — caught by the existing payload assertion, which is that test working. `SignInFirst` on `/onboarding` and `/create-team`, a component this repo had already written for this and not used on these two routes (the 1-in-24 registration flake). `MEMBER_REQUIRED_ATTESTATIONS` deleted on its own entry's terms. **The fixture sweep, and the trap it fell into.** `globalSetup` now sweeps `team_number ~ '^3[0-9]{4}$'`; ranges verified disjoint first; proved both ways with three orphans planted at the exact numbers `fixtures.ts` hands out — duplicate key and **20 tests skipped** without it, 20 passed and zero rows left with it. **The first version used `shell: true` with the pattern in an argument and cmd ate the `^`**, which is `docs/environment-divergences.md`'s own warning, reached by writing new code rather than by reading it; it surfaced at once only because the sweep fails loudly instead of swallowing its error. **Parking lot: seven entries closed, two opened** (GitHub forcing Node 24 on `checkout@v4`/`setup-node@v4`; the byte-identical-bundle check being unrunnable as written, since CI builds with the production secrets and a local Gate build does not). |
| 2026-08-24 | **Parking-lot triage — 161 entries, and the ten that had stopped being true** | `v2/gate-clock-freeze` | **Complete.** A disposition pass over every §8 parking-lot entry, written up in `docs/parking-lot-triage-2026-08-24.md`. **The Gate was red before it started, on `origin/main`, and had been for a day**: `operator-expiry-view.test.tsx` pins its fixtures to a frozen `NOW = 2026-08-23` and then renders `OperatorConsole`, which dates its own rows from the REAL clock (`directoryLoadedAt = Date.now()`). The two were the same instant on the day it was written and a day apart on 2026-08-24, so "3 days left" rendered "2 days left" and the file went red without a line changing — `docs/failure-modes.md` §10, in a file whose own header explains that the pure helpers take `now` as an argument precisely so this cannot happen. The component layer never got the same treatment. Frozen with `vi.useFakeTimers({ shouldAdvanceTime: true })` (required, not decorative — `waitFor` polls on real timers and a fully frozen clock deadlocks the render helper rather than failing it), and **watched failing both ways**: red before the fix, and red again with the frozen instant nudged forward one day, which is what proves the freeze is load-bearing rather than incidental. Gate green afterwards (lint / unit / integration / build, `GATE_EXIT=0`, precache 1,582 KiB). **The triage's own headline is that ten entries are already fixed and still read as open, two of them 🔴** — including "the backup has never run once", which is superseded by the release row immediately below and was never struck through. Taken at face value it is the most alarming line in the parking lot; `gh run list` settles it in one call (**scheduled run green at 2026-08-24T08:02:30Z**). All ten now carry their evidence and their original text behind "Was:". Also recorded: **11 entries describing 5 defects** (`lastError` twice, the trial licence twice, `teams`-not-in-the-registry three times and all three already resolved), 14 items that are Kevin's decision rather than work, and five one-line fixes with disproportionate value — the `"[object Object]"` `lastError` that SYNC-10 would ship verbatim, and a **ninth** display-initials implementation in `CheckIn.tsx` that disagrees with the canonical one (`MJ` where `member-utils` says `MW`). Open count after the pass: **~115**, not 161. |
| 2026-08-23 | **Production release — 86 commits, 14 migrations, and the backup that had never run** | `main` | **Complete, pushed and deployed.** Production had been seven sprints behind: 86 unpushed commits, **14 migrations the hosted project had never seen**, and a nightly backup workflow that `gh` answered `HTTP 404: workflow not found on the default branch` for — because `backup.yml` landed in Sprint 14 and `main` had not been pushed since `c1cec81`. **REHEARSED BEFORE APPLIED**, in the order `deploy.yml` already prescribes ("schema changes are ordered by hand"): manual dump → restore into a scratch database → run all 14 against the restored copy. That found the one real blocker with no risk: **production held two teams and both had team number 30727** (`TestTeam` and `Millennium Falcons`, both Kevin's), so D3's `UNIQUE (program, team_number)` guard refused — exactly what its fail-loud DO block was written for. Without the rehearsal the run would have stopped at migration 8 of 14 with 7 applied and the bundle mismatched. Kevin's call: clear `TestTeam`'s number (the partial index deliberately allows a team with no number). Applied to production with `db query`, one row, verified by `RETURNING`. Then `db push`: **all 14 applied, no errors**, 31/31 remote, data intact (teams=2, members=3, meetings=29, tasks=2, users=2, operators=1), 19→24 public tables, and `schema_assertions.sql` **passed against production**. Pushed; CI and Deploy green; falcon-forge.com 200 with CNAME intact and the bundle pointing at the hosted project. **TWO THINGS THE REHEARSAL AND THE FIRST BACKUP RUN FOUND, both of which had been believed working.** (1) CI's coverage gate refused the release at branches **59.96% against 60%** — and what it was pointing at was `GuardianView.tsx` at **ZERO coverage across 535 lines**, having just been given an irreversible "Remove this child" button in Sprint 21. First time that gate has caught something real. Thirteen tests added (7 GuardianView, 6 operator-console refusal paths); thresholds ratcheted 66/60/63/68 → 68/60/63/70. (2) The nightly backup **still does not work**, and `docs/beta-ops.md` is why: its secrets table said "Use the direct connection, not the pooler", but GitHub runners have no IPv6 and Supabase's direct host is IPv6-only on the free tier, so `pg_dump` can never reach it from Actions. That line was half right — `pg_dump` does need a session and the TRANSACTION pooler (6543) cannot give one — and it wrote the usable option out of existence: the **SESSION pooler on 5432** is IPv4 *and* session-mode. Kevin set the secret correctly according to the documentation. `backup.yml` now refuses in its first step if the URL is not a pooler string (9s and a named cause, against 34s and an IPv6 error after a 300 MB image pull). Kevin set `SUPABASE_DB_URL` to the Session pooler URI, and the run after that failed a THIRD way: the 50 KB size floor rejected a good backup, because production's data half is **38 452 bytes**. A byte count fails a small database that is perfectly backed up and passes a large one whose rows are missing — so it counts `INSERT` statements now and requires `auth.users` and `public.teams` by name, which is the check `beta-ops.md` already documented and called "the check worth doing on any backup". **Run 32670045597 is green: 38 452 bytes, 17 INSERT statements, the same 17 tables as the manual dump that was verified by restoring it, encrypted and retained 30 days. The nightly runs at 07:10 UTC from here — the first working backup in this project's history.** Still unproven: nobody has decrypted an artifact and restored it. |
| 2026-08-23 | Sprint 21 — SEC-11, the erasure the policy promises | `v2/sprint-21-erasure` | **Complete.** Every Gate stage green individually (lint / 1031 unit +2 skipped / 95 integration / build / schema assertions / **642** db / 418 rls), e2e 35/35, and an 18-check browser probe against a real build and a real database. One forward migration, `db:types` regenerated. SEC-11 has no exit-criteria block (checked), so the definition of done is the report's; the team JSON export, invoices, seat reduction and rename from its fix direction are parked, because the policy promises an export only as "ask us" while erasure was promised as something that just happens. **This encodes `beta-ops.md`'s runbook rather than inventing a procedure** — its SQL had been run against a real database and measured, and its order is load-bearing: `team_members` has five composite FKs pointing at it with SET NULL and FOUR CANNOT FIRE, because SET NULL nulls every column in the key and each tries to null a NOT NULL `team_id` (one of them `teams.id` itself). **THREE THINGS THE RUNBOOK GOT WRONG, none findable by reading it.** (1) "Then delete the login in the dashboard" does not work for most people: measured, `deleteUser` on a team owner is REFUSED and on a student succeeds, because `public.users` cascades from `auth.users` and four NO ACTION references refuse it for anyone who ever owned a team or issued an invite — every admin. The login is BANNED instead, one outcome for everybody. (2) The anonymisation WAS NOT DURABLE: it wrote the tombstone to `public.users` only, and `handle_new_user` fires on UPDATE of `auth.users` saying `email = EXCLUDED.email`, so the next password reset copied the real address back — an erased administrator kept a working login that silently un-erased itself. The function anonymises `auth.users` FIRST and lets the trigger carry the tombstone. (3) SEC-01's admin-protection trigger refuses both functions, so deleting a team was refused for EVERY team that has an admin — **and a psql probe of it SUCCEEDED**, because psql connects as `postgres` and the trigger's first bypass exempts exactly that. The probe agreed with a broken function; only the test calling it as the app does showed the refusal. A new `falconforge.operator_removal` flag licenses it, deliberately not reusing `admin_transfer`. **`operator_actions.team_id` was NOT NULL with ON DELETE CASCADE**, so deleting a team deleted the record of its own deletion; it is nullable and SET NULL now, with the team's identity copied into `detail`. The erasure row records the SHAPE of what went and NOT the name or address, with a test that fails if either appears. **Erase is ABSENT on a child's row, not disabled**: a managed member's `user_id` is their GUARDIAN's, so it would erase the parent and every other child they have. The sole admin's is DISABLED with the remedy in its title, which is the opposite case and the difference is the point. The guardian's removal mirrors the cascade LOCALLY — otherwise the child's name goes while their membership and attendance keep counting from rows that outlived them — with a sibling control, because "removes everything" passes every other assertion in that file. **The Privacy Policy described a button that has never existed** ("when you delete your account"); it says "when you ask us to" now. Watched red three ways on the RPCs, once on each UI guard. **The browser probe found its own defects first**, all three reporting success on a broken feature: it had selected the WRONG TEAM (the search box is inside a form and filling it does not submit) and reported the child guard as a pass on absence-from-the-wrong-panel; and two checks compared 0 to 0 because the seed creates no tasks. Also fixed a real defect it exposed: `seed-review-states.mjs` discarded the error from `deleteUser`, turning a refusal into a misleading "already registered" four lines later. Two intermittents recorded rather than re-run until quiet — a 14 759 ms unit timeout (1 in 6 full runs, passes alone and 5 consecutive after) and one recurrence of Sprint 18's parked redirect defect. |
| 2026-08-23 | Sprint 20 — SEC-17, SEC-13, WALK-A-12 | `v2/sprint-20-credentials-and-cleanup` | **Complete.** Every Gate stage green individually on the final state (lint / 1021 unit +2 skipped / 95 integration / build / schema assertions / **632** db / 418 rls), e2e 35/35. One forward migration, `db:types` regenerated. Chained `gate:db` still aborts at 127 — Sprint 19's parking-lot entry, which reproduces on `1610c0f`; it hit twice here and each time left the fixture teams the OTHER entry describes. None of the three IDs has an exit-criteria block (checked, after getting that wrong in Sprint 19's first draft), so the definitions of done are the report's. **SEC-17: there were two invite-code generators** — `Math.random()` over 32 confusable-free symbols in the panel, `upper(substr(md5(random()::text),1,8))` in `create_team_as_admin`: different alphabets, ~40 bits against ~32, and a support call about a bad code could not be answered without knowing which screen made it. The same shape SEC-09 already found in this table's `expires_at`, which is why the fix is a column DEFAULT and not a shared helper — a helper is still two call sites. **The length was never the defect**: an invite code is a bearer credential and V8's `Math.random()` is xorshift128+, whose state is recoverable from a handful of outputs, so a member could predict another team's next codes. `gen_random_bytes` now, with `& 31` written as a mask so the power-of-two assumption is visible — a non-power-of-two alphabet makes that line silently biased and every code would still look random. **Two first-draft errors, both caught rather than reasoned about**: `REVOKE INSERT (code)` against a role holding the TABLE privilege is a NO-OP and `has_column_privilege` still answered true — a control that reads as applied and does nothing, which is why the test issues the write instead of reading the catalogue; and revoking `anon` outright broke `db:verify`, correctly — this repo has ONE boundary, RLS, and grants only answer "can PostgREST use this table". `code` stays revoked from both roles because that is a different question, and assertion 6 asks `has_any_column_privilege` precisely so a deliberate column narrowing can coexist with it. SEC-17's other two parts: the orphaned preflight is **deleted** (it preflights a migration in `_archive/pre-v2` that the deployed commit `c1cec81` does not carry), and the assertion-23 rationale was **already correct** — verified, not fixed. `as any` **56 → 55**. **SEC-13: the enumerating message was dormant, not absent.** "An account with this email already exists" cannot fire with `mailer_autoconfirm: false` (both environments), so the leak sat one dashboard toggle away — SEC-14's class exactly. Both branches now share one constant, and the test renders both paths and requires the strings to be CHARACTER-IDENTICAL, because a pattern match is satisfied by two different strings and a trailing full stop is enough to be an oracle. **The old test asserted the finding** and would have passed whichever way the toggle went. **WALK-A-12: not a defect, measured.** 60 KB of roster with names and email addresses while signed in, every store at 0 and localStorage empty after — and the database surviving is Dexie keeping its schema, now asserted so nobody "fixes" it. The control (rows must exist first) is the point; the probe's own content check was broken on the first run and reported a false FAIL. No parking-lot entries. |
| 2026-08-23 | Sprint 28 — the Training UI stub (D5) | `v2/sprint-28-training-stub` | **Complete.** Gate green stage by stage: lint / **1182 unit** +2 skipped / 99 integration / build, e2e 40/40, coverage 69.89/62.47/65.97/71.69. **No migration** — that is D5's own boundary and the line the sprint was scoped by; `supabase/` untouched, so `gate` rather than `gate:db`. Ratchets unmoved: `as any` **51** at its ceiling, arbitrary Tailwind values **2**. D5 has **no exit-criteria block**, so `docs/sprint-28-report.md` states five criteria this sprint wrote, from the decision's own words, and quotes each with how it was checked. **What shipped**: one nav entry visible to everybody on a team and three deep-linkable routes (tracks → one track → one lesson); a lesson carrying objective, hands-on task, prerequisites that link across tracks, learner time, one of five checkpoint verifiers, and flags for team-authored, rewritten-every-season and gates-build-work; a sign-off queue and a per-lesson "who has done this" for an adult; and empty states designed for the case that is true — **11 of 61 lessons outlined**, six of the eight tracks carrying only their planned count. Tracks A and B are transcribed from `training-onboarding-design.md` §2.1 and were chosen because between them they instance every shape the screens render: all five verifiers, a team-authored lesson, a game-specific one, a build gate, a prerequisite chain and two lessons with no hands-on task. **Progress is deliberately not recorded, and the control says so under itself rather than being hidden** — a local-only store key would have cost ten lines and been wrong twice: a second data path for something that must end as a synced entity (principle 3), and a tick a student can see that their mentor cannot. **The student/mentor split reuses the app's existing role rule rather than inventing a second**: the predicate moved to `src/lib/roles.ts` and `canManageMeetings` now calls it; the two capability NAMES stay separate because they coincide today and are not the same question. **The defect found by running it: the archived-season banner followed Training** — two amber banners stacked, the upper one saying the season is read-only on the one view a season never touches. `ArchivedSeasonBanner`'s own docblock named the cause ("every view is season-scoped"), true when written and false since `help`, `profile` and the guardian view arrived. Fixed on the view definition (`seasonFree` + `pathShowsSeasonBanner`, the same construction as `pathNeedsTeam`) rather than as a Training-only exception, and verified in the browser with the season genuinely archived in Postgres: absent on training/help/profile, present on board/dashboard/scouting. **A second, found while seeding a review team**: `scripts/seed-demo-team.mjs` still wrote `tags: []` and died on Sprint 27's dropped column — one line, fixed here. **Twelve reverts, each watched red**, including two written specifically because the off-season assertion looked like decoration on first reading (a season gate in the component, and the banner rendered unconditionally). The component tests render the WHOLE app, because two of D5's four questions — is it reachable, does it survive the off-season — are properties of the shell and the route table. Browser verification on a `--mode development` build at 375/600/1280 px, as a student and as an admin, with the role changed in Postgres between runs: no horizontal overflow at any width, cards 1/1/3 columns with equal heights per row, lesson rows ≥60 px. Two parking-lot entries. |
| 2026-08-23 | Sprint 27 — consistency and dead code (FEAT-06, FEAT-07, FEAT-09, FEAT-13's dead-field half) | `v2/sprint-27-consistency` | **Complete.** Gate green stage by stage: lint / **1148 unit** +2 skipped / 99 integration / build / schema assertions / 667 db / 418 rls, e2e 40/40, coverage 69.56/62.14/65.43/71.38. One migration. **`as any` 54 → 51, and the CEILING lowered from 55 to 51** — the gain locked in rather than left as headroom. Each ID had a scoping trap where the fix direction points at deferred work, and all four are taken the way the handoff scoped them. **FEAT-06**: the d3 drag behaviour was bound to `.draggable-robot` and no element with that class has ever been rendered — the effect ran on every `isDrawingEnabled` change and selected nothing. Deleted rather than implemented (game-piece tokens are P-01 phase M and unscheduled; code kept against an unscheduled feature is failure-modes section 7 pointing the other way). **It was the only d3 import in the codebase**, so the dependency, `@types/d3` and the `charts` manualChunk went too: precache **1583.05 → 1558.09 KiB**. Two further casts went with it — the pointer-capture calls never needed one. **FEAT-07**: the help page told every new student "Drag a card as the work moves" and the board has no drag-and-drop, has never had any, and has no DnD dependency. Reworded, and held by TWO ratchets — the second asserts no DnD library exists, so that if somebody adds drag the failure says "this ratchet should go" rather than sending them to delete a word from a sentence that has become true. **FEAT-09**: two definitions of progress on one screen — the sidebar counted Backlog and Archived, the dashboard did not, so archiving a Done task LOWERED one and left the other. One `sprintProgress(tasks)`, both read it, done standalone rather than folded into P-03. **Measured in the browser**: three tasks showed 2/3 in both places, and archiving one Done task moved both to 1/2 — where before the sidebar would have gone to 1/3 and the dashboard to 1/2, from the same click. The agreement test renders the WHOLE APP and reads both numbers, because a shared selector only one caller uses would pass every assertion in the unit file. **FEAT-13**: `tags` was written as `[]`, round-tripped both ways and stored in a NOT NULL `text[]`, with no input, filter or display anywhere. Removed from the type, the registry, the modal, **22 fixtures across 18 files**, and the column dropped — measured first, because "nothing ever wrote it" is a claim about data: production has 2 tasks and 0 with a non-empty `tags`. Only the dead-field half; the sprint entity is P-03 and not started. **The environment trap met in practice**: `git checkout package.json` silently destroyed the d3 removal, because it restores from the INDEX and the file was unstaged — the fifth time in this project, and `HANDOFF_BUILD.md` warns about it by name. No parking-lot entries; every discovery was inside its own IDs. |
| 2026-08-23 | Sprint 26 — scouting that answers questions (P-02 minimal set = FEAT-15) | `v2/sprint-26-scouting` | **Complete.** Gate green stage by stage: lint / **1136 unit** +2 skipped / 99 integration / build / schema assertions / **667 db** / 418 rls, e2e 40/40 on the second run, coverage 69.53/62.10/65.34/71.35. One migration (two nullable columns), `db:types` regenerated. P-02 HAS an exit-criteria block, so every clause is quoted in `docs/sprint-26-report.md` with how it was checked. **THE FINDING WITH NOTHING TO DO WITH SCOUTING: the unit and integration suites have been talking to PRODUCTION.** Vite loads `.env.local` in every mode including `test`, `.env.local` points at the live project, and so `src/lib/supabase.ts` built a real client against it in every run — while `setup-integration.ts`'s own docblock said both clients were null "(no VITE_ credentials in the test environment)". **Measured, not assumed: nothing landed** — every write was refused by RLS for want of a session, and production reads 0 scouting reports and 2 tasks, none matching a fixture. Third recorded instance of env-divergences section 2, first as the test suite rather than a script. It surfaced because this sprint's new columns turned an RLS refusal (42501, retryable) into `PGRST204 — Could not find the 'alliance' column ... in the schema cache` (terminal), so a sign-out test found a parked change where it expected a re-queued one: the suite had noticed the schema of a database it should never have reached. Both configs now empty the two VITE_ vars explicitly, with an invariant that reads the VALUES rather than the key names. **P-02 itself**: one row per team with a column per `scoring.metrics` entry and the population σ beside every mean, sortable; a team detail listing that team's reports; alliance and station on the form (both optional, station count from `match.allianceSize` so FRC needs no code); CSV export of what is on screen. Nothing game-specific in any of it. **Verified offline in a real browser** with fetch blocked and `navigator.onLine` false: the table still reads `30±10.0` and the export still produces a file, because both are computed on the device. **Three reverts because the fix has an obvious wrong version**: nulls sorted as zero (a team nobody scouted at the top of the "best auto" list), `Number(x) || 0` (the idiom that put B18's five fabricated zeroes into production scouting data), and a metric column starting ascending. **Two ratchets earned their keep**: `dark:text-slate-500` caught this sprint's own new code, and the tap-target assertion read 32px in one browser and 30px in another because `touch-target` is gated on `pointer: coarse` — WALK-A-10's recorded artefact, met from the other direction. **A behaviour change stated plainly**: the page opens on the SUMMARY, so 12 unit tests and one e2e spec now switch to the cards view first; each still asserts what it asserted. One intermittent recorded: `meetings.spec.ts:333`, Sprint 18's diagnosed route-guard redirect, passed alone and 40/40 next run. Two parking-lot entries. |
| 2026-08-23 | Sprint 25 — the sync engine (SYNC-13, SYNC-09, SYNC-04, SYNC-06) | `v2/sprint-25-sync-engine` | **Complete.** Every Gate stage run individually and green: lint / **1101 unit** +2 skipped / **99 integration** / build / schema assertions / **661 db** / 418 rls, e2e 36/36, coverage 69.26/62.04/65.01/71.22 — up on all four. No migration. `as any` 55 → **54**. **Every B1–B26 regression test is green; two had to CHANGE and that is reported rather than buried** — `DrainResult` gained a `stalled` field and two whole-object `toEqual` assertions noticed, which is what a whole-object assertion is for. Both gained the field; neither was relaxed to `toMatchObject`, because that trades a real property for never having to touch the line again. **SYNC-13**: the 30 s budget now measures IDLE time, not total, and only a successful push resets it — a device back from a day offline no longer reports "Sync failed" after pushing sixty items and being cancelled mid-drain. A stall that pushed nothing still errors; a stall that pushed something shows "N pending" in amber. **SYNC-09**: `navigator.locks` with `ifAvailable`, so a second tab declines rather than waits — verified in the real browser, where an edit made with the lock held stayed queued with **retryCount 0** and drained the instant it was released. **SYNC-04**: a tab hidden for two minutes releases its socket and reconnects-then-pulls on return; the constraint is CONNECTIONS (200 concurrent ≈ 15–25 teams meeting at once), not messages. **SYNC-06, the big one**: an update now sends only the columns it changed, diffed by running `toRemote` over the pre-edit and post-edit rows — no second key map to drift, no `toRemotePartial` on eleven entities. Coalescing keeps the EARLIEST pre-edit row, or three offline edits collapse into a push of only the last one. **Three reverts exist because the fix has an obvious wrong version**: reference comparison instead of value (`checklist`, `timeline`, `tags` and scouting `data` are objects, so `!==` makes the diff the whole row and the fix a no-op — green tests, defect intact); newest-instead-of-earliest base on coalesce; and a deadline that resets on every item rather than on every success. **Verified through the real UI**: offline rename of a task whose status device B had moved to Done → the server row reads `Conflict probe RENAMED | Done`, where before it would read `… | Backlog`. **Principle 3 worked out and written down**: the pending guard is deliberately NOT loosened to per-field, because the window it protects is the user's own work in progress and it closes by itself the moment the push lands — B3 and B8 are green UNMODIFIED, which is the evidence. **Three things decided rather than built**, each with its number: SYNC-04's table trim (36 bindings measured, not 24; dropping `seasons` would undo B22), SYNC-06's `updated_at` precondition (needs a conflict path, or a silent revert becomes a dead-letter a coach cannot act on — same-field edits stay last-write-wins), and SYNC-09's per-tab timestamp allocator (ties are only possible between different records). **Three of SYNC-13's own test drafts were wrong in ways that looked like the fix failing, and one PASSED with the fix reverted** — 11-second refusals never spent the budget because the successes between them kept resetting it. Caught by the revert pass. One parking-lot entry. |
| 2026-08-23 | Sprint 24 — the guardian edges (SEC-15, SEC-16) | `v2/sprint-24-guardian-edges` | **Complete.** Every Gate stage run individually and green: lint / **1091 unit** +2 skipped / 95 integration / build / schema assertions / **650 db** / 418 rls, e2e 36/36, coverage 68.93/61.65/64.80/70.83 — up again on all four. One forward migration, no type regeneration. **Neither ID has an exit-criteria block**, so both definitions of done are this sprint's, in `docs/sprint-24-report.md`. **SEC-15**: `join_team_with_invite_for_child` refuses unless the profile and its consent exist SERVER-side, and both are written through the sync queue — so the flow plan section 3 describes, add a child and enter the code in one sitting, races the drain and a parent got "This child has no consent on record" ten seconds after ticking the box. One drain before the call, only when something of THIS child's is queued; if the rows are still queued afterwards the RPC is not called at all and the message names the child and says nothing was lost. **The ordering assertion is the one that matters** — two separate `toHaveBeenCalled` checks pass whether the drain runs before or after the RPC, and the revert proved this one can tell (`expected 43 to be less than 42`). **SEC-16, two halves.** A guardian's EMAIL change never reached their child's roster row: the trigger filters on `managed_profile_id IS NULL` so renaming yourself does not rename every child, which is right, and the email went stale silently as a result. A second UPDATE now carries the email only — `full_name` and `avatar_url` stay excluded because both belong to the child, so the distinction is kept and applied per column rather than per row. And **`operator_erase_user` contradicted a locked decision**: plan section 3 says a promoted child's `managed_profiles` row and consents are retained as the record of why they were rostered, and SEC-11's tool deleted them — so the team kept the member and lost the COPPA evidence trail. The DELETE is now `promoted_to_user_id IS NULL`, with `notes` cleared on the retained row. Both function bodies are `pg_get_functiondef` output with one statement patched, and the generator asserts its anchor appears exactly once. **The finding underneath it all**: `Fixtures.createGuardian` inserted the child's `team_members` row with NO email, so every db test using it worked against a row production never produces — and SEC-16, whose whole subject is that address going stale, had nothing to go stale. Verified in the browser with `fetch` blocked for the two tables' writes only: the profile and four consents sat queued, the button read "Saving Robin Junior's profile…", the join was refused with the honest message, and restoring the network pushed all five rows and joined. One parking-lot entry. |
| 2026-08-23 | Sprint 23 — edits that are silently lost (FEAT-03, FEAT-04, FEAT-10) | `v2/sprint-23-lost-edits` | **Complete.** Every Gate stage run individually and green: lint / **1086 unit** +2 skipped / 95 integration / build / schema assertions / 645 db / 418 rls, e2e 36/36, coverage 68.81/61.53/64.62/70.71 — up on all four. No migration: all three are client defects sitting beside a server that was already right. **None of the three has an exit-criteria block** (checked by ID and by subject), so the definitions of done are this sprint's, in `docs/sprint-23-report.md`. **FEAT-04 is the one worth reading.** The handlers did `const c = [...task.checklist]; c[idx].completed = ...` — a shallow copy of the ARRAY whose item objects are the store's, because `App.tsx`'s route adapter copies the task and its timeline and not its checklist. So Cancel could not revert: the tick stayed on screen, was never queued, and vanished on the next pull — a student ticks "wiring checked", closes the dialog, and the item silently un-ticks itself later, on their device only. **The test's fixture is built exactly the way the route adapter builds the prop, deliberately**: a deep-cloned task would have passed against the mutating version, which is §2 in its usual form. **FEAT-03**: a comment typed into a new task appeared in the feed and was discarded by Save, because `saveTask` passed no `timeline` and `addTask` built a fresh one holding "Task created" and nothing else. Threaded rather than hidden — the first thing anybody says about a new task is usually why it exists — and "Task created" goes LAST, because the feed renders newest-first and every other writer prepends. **FEAT-10**: the local cascade omitted meetings and attendance while the docblock above it said the server removes meetings, and the server does — so the rows went from the database and stayed in the store, pointing at a season that exists nowhere. Attendance is queued BEFORE its meeting (it references `meetings (id, team_id)`, and an error in the drain is five retries and a dead letter), and the confirm dialog lists seven things rather than five. **Three reverts exist so the fix cannot be half-done**: swapping the two cascade entries still deletes everything locally and fails only the ordering test; putting "Task created" first still keeps the comment; and the Save-still-saves assertions catch a fix that made Cancel work by breaking Save. Verified in the browser on a real build — comment survives a reopen and reached the server (timeline length 2, newest first); tick unticked after Cancel, ticked after Save, confirmed server-side; and the real cascade took **17 meetings and 122 attendance rows** out of the persisted IndexedDB blob with a queue depth of 0 and zero failures. No parking-lot entries — every discovery was inside its own IDs. |
| 2026-08-23 | Sprint 22 — the September list (WALK-B-06/07/10, FEAT-14, OPS-08/SYNC-12, OPS-12, OPS-14/15, the restore) | `v2/sprint-22-september-list` | **Complete.** Every Gate stage run individually (`gate:db` still exits 127 part-way on this machine) and green: lint / **1074 unit** +2 skipped / 95 integration / build / schema assertions / **645 db** / 418 rls, e2e **36/36** across chromium and mobile, coverage 68.64/61.41/64.25/70.51 against floors 68/60/63/70. One forward migration; no type regeneration (a CHECK adds no columns). Ratchets unchanged at 55 `as any` and 2 arbitrary Tailwind values. **None of the eight IDs has an exit-criteria block** — checked by ID *and* by subject, so not Sprint 19's missed-shared-bullet mistake — so all eight definitions of done are this sprint's, stated in `docs/sprint-22-report.md`. **The finding worth reading first: the WALK-B-06 fix was still wrong after five green tests, and running the app said so in numbers.** Signing in on a cold device sampled the header at 102 ms showing *"Nothing has been added yet"* to a team with a season of work in it, and "Welcome back" at 212 ms. There are TWO kinds of "not yet" — IndexedDB rehydration, which `useStoreHydrated()` covers, and the first SERVER pull, which it does not — and every one of the five tests had the store already populated when it rendered (`failure-modes.md` §2 exactly). Gated on `isLoading` as well; re-measured at 26 ms neutral, 301 ms "Welcome back", new-team copy never shown to a team with work. **WALK-B-07**: two badges truncating differently (`slice(0, 2)` and `slice(-3)`) — team 30727 was `#30` in the rail and `#727` in the picker, both valid-looking numbers belonging to other teams. The truncation existed *because of the layout*, so the fix is a pill that grows rather than a deleted `slice`, with no `truncate` (a clipped team number reads as a different team). Geometry held by a new e2e spec, watched red with the badge restored to a fixed 28 px circle. **WALK-B-10**: the ninth name column, and the only one an adult types on behalf of a minor; forward migration rather than an edit to the Sprint 19 one, and the drift test now reads BOTH migrations. Typing the walkthrough's own 142-character emoji name through the browser's editing path now yields 120. **FEAT-14**: rename in place with the draft in component state — writing it through to the store is FEAT-04's shape, live in `SprintTaskDetail` today, and would have shipped the same defect twice; the Cancel and Escape tests are red exactly when that is done. **OPS-08/SYNC-12**: precache **5213.30 → 1583.05 KiB** and `dist/` **5,357,696 → 1,640,120 B** (69.6%). Four files shared md5 `f74e417a` — one 1024² PNG under four names, 3.2 MB of a 5.2 MB precache — and `icon-192.png` had been *declared* 192×192 while being 1024² since the project started. Manifest 49 entries for 46 unique URLs → 45 for 45 (`includeAssets` named two files that do not exist plus one the glob already had; `includeManifestIcons` added the icons a second time). Three new invariants, all watched red, and the icon one reads the PNG's IHDR so it cannot be satisfied by the declaration it checks. **OPS-12**: 19 → 18 findings, 4 → 3 with `--omit=dev`; react-router-dom 6.30.3 → 6.30.6, and `readReturnTo` now refuses a backslash (GHSA-wrjc-x8rr-h8h6 is the bypass of the `//` check it already had — unreachable through a HashRouter, refused anyway because the unreachability is a property of the router). supabase-js **not** taken to 2.112 and pinned at 2.89.0 instead: it breaks the typing of `sync.ts` and `server-pull.ts` (six `tsc` errors), and the `ws` advisory it closes is a Node transport absent from all 34 built assets. **OPS-14/15**: five runbooks written, the CLI pin made exact and held by a new invariant. **The restore is now counted rather than believed** — `scripts/restore-rehearsal.mjs` over today's hosted dump: 0 psql errors, **65 of 65 rows**, table by table. Its own first run reported OK over `0 expected / 65 restored`, which is §2 inside the script written to prevent §2. **And the documented `session_replication_role` trap did not reproduce**: the `SET` succeeds as `postgres` despite `rolsuper = f` and `has_parameter_privilege` false, because `supautils` allow-lists it; the hosted project allow-lists it too but under a different privileged role, untested because testing it means a SET against production. The trigger-disable blocks stay. Eight parking-lot entries. |
| 2026-08-23 | Sprint 19 — WALK-A-08/09/10/11, the accessibility set | `v2/sprint-19-accessibility` | **Complete.** `gate:db` green on the branch at `86cece7` (lint / 1021 unit +2 skipped / 95 integration / build / schema assertions / 626 db / 418 rls), e2e 35/35. **`gate:db` later began dying part-way through `test:db` with exit 127 — not a Sprint 19 regression, it reproduces on `1610c0f`; see the parking lot — so `main` was re-verified stage by stage rather than in one chained run, every stage green.** One forward migration; no type regeneration needed (CHECK constraints add no columns). Two new browser probes, `probe-accessibility.mjs` (axe-core wcag2a+aa, 12 checks) and `probe-long-titles.mjs` (8 checks), both green against a freshly reset and reseeded stack. All four share **one** exit-criteria bullet (`exit-criteria.md:158`) and it is met and exceeded on every clause — zero `select-name`/`button-name` on eight routes rather than the two named, the slate-500-on-dark tokens at 5.70/8.08/6.97:1 against a 4.5:1 bar, `min(w,h) >= 32` rather than height alone. The bullet's heading says "wrapping" and its text does not cover it, so **WALK-A-11 alone is measured against a definition of done this sprint wrote**, stated in `docs/sprint-19-report.md`. (That report's first draft claimed all four had no criteria; corrected.) **The finding worth reading first: 64 of WALK-A-10's 123 sub-32px controls were a measurement artefact.** `touch-target` is gated on `@media (pointer: coarse)`, and a desktop Chromium resized to 375px matches `fine` — so the walkthrough (and this probe's first run) measured a media query that was switched off. The four sidebar controls the finding names by number were already correct on a real phone. `docs/failure-modes.md` §11, invisible in both directions: both runs produce a plausible list of small boxes, and had the fix landed without noticing, the probe would have reported *no change* and the obvious next move is an ungated rule that wrecks the desktop. **WALK-A-08**: focus/trap/Escape live once in `Modal.tsx` for fifteen call sites; two modals deliberately take no `onClose` and keep the trap without dismissing. Its own test caught a real error — capture-phase + `stopPropagation` does **not** order two listeners on the same node (`document`), so one Escape closed a stacked confirm *and* the form under it; a module-level stack answers "am I innermost?" explicitly. No backdrop dismiss: most modals here are forms, and a mis-click beside the scouting dialog would discard a report a scout just typed. **WALK-A-09**: 24 contrast nodes and 15 nameless controls → **zero**, across nine scans including the task modal, which the walkthrough explicitly could not reach. Both name bugs have the same shape — `aria-label={showLabel ? undefined : label}` looked like the careful choice (don't repeat a name the user can see) and left the switch with no accessible name at all, because the visible text is a *sibling* span; and `text-slate-400 dark:text-slate-500` looked careful (dimmer text on a dimmer ground) and inverts the requirement. New ratchet at zero: slate-500 measures 3.07/2.52/2.18:1 on the three dark grounds this app uses, so it needs no knowledge of the ground to judge. The probe also found a defect nothing else had — Sprint 18's new-event date input has no label and a `type=date` has no placeholder to fall back on; the Gate was green for it twice. **WALK-A-10**: 123 → 59 → 15 → **0**, via one element-selector min-height rule (not the class-substring rule index.css removed, and not min-*width*, which is what squashed the board's row actions). Two defects introduced and caught by *looking*: the floor stretched the toggle's track to 32px leaving its knob adrift (hit area and track are separate boxes now — 36×32 and 36×20, both asserted), and a 32×32 native checkbox dwarfed its 12px label while answering a question nobody asked, since every checkbox here is wrapped in its label and the label is the control. The probe now measures the label, and it re-checks the **desktop** to prove the floor did not leak — 51 controls still compact. **WALK-A-11**: `break-words` **alone did nothing** — the h1 still measured 1331px inside a 375px viewport, because a flex item defaults to `min-width: auto` and `overflow-wrap` only breaks text that has run out of room. Now 343px over four lines. The 120-character limit is written twice (a TS constant and a CHECK on all eight title/name columns) and a test reads the migration's CHECK *expressions* — not the digits, since the header comment says 120 too — and requires them to equal the constant; watched red both ways. Verified against real Postgres: 121 refused, 120 accepted, trailing spaces accepted (btrim first), 120 emoji accepted because `char_length` counts code points where JS counts 240, so the client is always the stricter of the two. The migration's fail-loud guard was tested by making it fire. One parking-lot entry. |
...
- **The field image is a crop of FIRST's own manual figure, and nobody has said we may ship it.**
  `public/BiobuzzField.webp` is Figure 9-2 of Competition Manual V1 (an orthographic render of the
  official CAD), cropped to the perimeter with the magenta callout arrows painted out. DECODE's field image
  had the same provenance question and it was never answered either. R-02 asked "under what
  licence"; this sprint did not answer it. If the answer is no, the alternative is a render from
  the STEP file (`BIOBUZZ_Full Field.20260912.step`, 35 MB) — which needs CAD tooling this machine
  does not have (no OCC / FreeCAD) — or a drawn diagram in the style of `DecodeField.png`.
- **A 2026-27 report scouted under the placeholder shows blank rows.** Any team that created a
  BIOBUZZ season between Sprint 18 and this release and scouted with it stored `hasAutonomous`,
  `autoScore`, `teleopScore`, `endgameScore`. The data survives (the registry passes the bag
  through opaque) but none of those keys is on the real form, card or summary. Almost certainly
  zero rows — the game was unrevealed until yesterday — but it is one `select` against production
  to know rather than assume: `data ? 'teleopScore'` on `scouting_reports`.
- **Eleven browser probes still wait for a team picker a one-team account no longer sees.** The
  kickoff pass's one-team change sends a single-team user straight in, and `waitFor` on
  `team-picker` times out: observed on `probe-events-and-form.mjs` (fixed here, it was needed to
  verify this sprint) and `probe-biobuzz.mjs` (written with the fix). The same line is in
  `capture-screens`, `venue-simulation` and nine `probe-*` scripts; each needs the two-selector
  wait these two now use. Not fixed wholesale because each probe's account differs and each should
  be run after the change, not assumed.
- **No summary column says what a team is worth in POINTS.** A metric is one field and one
  aggregate by design (`game-definition.ts`), so "estimated match points" — 20 per tip, 5 per
  park, 3 for leave — cannot be declared. It is the column a pick-list lead will ask for first.
  The design note already anticipates it: a derived-metric `type` on `GameMetric`, not an
  expression evaluator.
- **Tips, park and garden are alliance-scored and the form scouts them per robot.** "Hive tips
  caused" counts the tips a scout saw THIS robot's launch complete, which is a judgement, and two
  scouts on one alliance can both claim a tip. It is still the number the POLLINATOR RPs (4 / 7
  tips) turn on, so it stays; worth a line in any scouting-lead training.

*From the kickoff readiness fixes, `v2/kickoff-readiness-fixes` (2026-09-05):*
- **✅ DEFINITION AND FIELD DONE 2026-09-13 (Sprint 37); RELEASE STILL KEVIN'S.** Written from
  Competition Manual V1 Table 10-2, field image from its Figure 9-2, dry run done exactly as
  described below — see the Sprint 37 progress row. What remains is the release window itself and
  the image-licence question. Was:
  **🔴 R-02 — the BIOBUZZ game definition is still a placeholder, and it needs a RELEASE PLAN
  rather than a ticket.** `src/games/ftc-2026-biobuzz.json` carries `version: 0`, a scouting form
  of Has Autonomous / Auto Score / TeleOp Score / End Game Score / Rating / Notes, a partner
  capability ("End game position") that is a guess, and `field.image: DecodeField.png` — so a
  2026-27 season's Match Planner draws last year's field. It CANNOT be written before the reveal
  on 12 September, and P-01 phase M (the global definition table) was deferred, so there is no way
  to change it without shipping a build. Everything downstream of scouting — the summary table,
  the CSV, the match plans — is only as good as this file. Kevin's prework: who reads the manual on
  the 12th, what the fields and metrics are, where the field image comes from and under what
  licence, and the release window. **Do the dry run first**: edit the JSON, run the Gate (the
  harness validates a definition at build time), and see it render through the new-coach funnel,
  which is currently the ONLY way to see BIOBUZZ at all — `scripts/seed-review-states.mjs:191`
  labels the 2026-2027 season DECODE, so no seeded team exercises it.
- **R-01 has not been exercised in a running app, and the suite structurally cannot do it.**
  The refocus fix is covered by five jsdom tests, and jsdom never fires `visibilitychange` —
  which is precisely why the defect survived 1,200 of them. Docker was down when the fix was
  written and every screen it affects is behind auth, so nothing drove the real thing. The check
  is the one already written into the review's §5: on a real phone, add to home screen, sign in,
  start a scouting report, lock the phone, unlock, and confirm the half-filled report is still
  there. Until that has been done, treat R-01 as fixed-and-unverified rather than as fixed.
- **The attestation copy ships contradicting two locked decisions, by decision (R-03).** Kevin,
  2026-09-05: leave it. `CreateTeam.tsx` tells a coach they "will be billed monthly based on
  approved team members" (pricing is deferred under D1, and per-seat is the option the evidence
  argues against) and that they "will act as the parent's agent for COPPA purposes" (§3 ships the
  opposite model — the guardian consents directly and the admin only attests not to roster a child
  without them). This text is what `coach_terms@2.0` RECORDS, so beta coaches are attesting to a
  billing model that may never exist. Changing it is a version bump to 2.1 and re-attestation for
  every existing admin; today that is Kevin's two production teams, and it gets more expensive
  with every beta team onboarded. **The cheap moment is before the cohort lands, not after.**
- **Corporate and school mail scanners will spend the confirmation token before the coach clicks
  it** — not built this pass, by decision. The `token_hash` round trip is still unbuilt and beta
  coaches on district email are the likely cohort, so "link expired" is a plausible first support
  ticket of the beta. The interim answer is a support script: ask the coach to request a new link
  and open it on their phone.
- **Two dashboard test files rendered `DashboardHome` bare**, outside the `AppShell` outlet it
  actually lives in, and both broke the moment it asked the shell a question. Now one shared
  `src/test/render-dashboard.tsx`. Worth a sweep: any other component test that renders an outlet
  child directly is carrying the same latent break, and it only shows up when somebody adds the
  first `useAppShell()` call.
...
- **✅ FIXED 2026-08-25 (Sprint 33).** `npm run check:bundle` answers "which backend is `dist/`
  wired to?" as a STATIC check on the built files — no server, no browser, no Docker — so it can run
  before anything expensive or irreversible. `npm run preview:local` builds, checks and serves in one
  step, so the local path no longer depends on remembering `--mode development`.
  **It asserts the local stack is PRESENT, not merely that production is absent**, because a stale or
  broken build references neither and would pass the negative form (§4, the zero case). All three
  states were exercised: the production-wired `dist/` the Gate had just left (fails, naming the Gate as
  the cause), a `build:local` bundle (passes), and a bundle with the local reference stripped (fails).
  `docs/environment-divergences.md` §8 rewritten: it blamed `test:e2e`, and the Gate does this far more
  often. Was: **`npm run gate` silently repoints `dist/` at PRODUCTION, and I nearly reviewed the
...
- **✅ FIXED 2026-08-24 (Kevin's call: bump now, on a green build).** All nine uses across `ci.yml`,
  ...
  pins, so the first real breakage will arrive on a commit that changed nothing. Bumping both to
  `@v5` is a two-line change; the reason to do it deliberately rather than reactively is that
  `setup-node` is what the Gate runs inside.
- **✅ ANSWERED 2026-08-25 (Sprint 33)** — same root cause as the entry above, and now written into
  `docs/environment-divergences.md` §8: a local Gate build and a CI build differ BY CONSTRUCTION, since
  CI builds with the production `VITE_SUPABASE_*` secrets, so a byte-identical comparison between them
  can never pass. The check that means something is what the live bundle POINTS AT, which is what the
  last three releases were verified with. Was: **"The live bundle is byte-identical to the local build" cannot be run as written, and the
  attempt on 2026-08-24 proved it.** Local `dist/` was `index-B6koE9AR.js` and the live bundle was
  `index-C51n5prB.js` — not a deploy problem at all: CI builds with the production
  `VITE_SUPABASE_*` secrets and a local Gate build does not, so the two differ by construction and
  the check can only ever fail. This is the Sprint 8 entry further down predicting itself. The
  version that means something, and the one actually used to verify this release, is to check what
  the live bundle POINTS AT — `curl` the entry chunk and grep for the hosted project ref, then
  confirm no `.map` is served. Worth folding into `check-production.mjs`, which already runs on
  every deploy and is the natural owner.

*From Sprint 28 — the Training UI stub, `v2/sprint-28-training-stub` (2026-08-23):*
- **`training-onboarding-design.md` §2.1 says "65 modules"; its own tables list 61.** Counted
  per track: A 7, B 4, C 10, D 8, E 16, F 5, G 6, H 5 = **61**. The section's closing paragraph
  ends "Totals: 65 modules", and every downstream number in that document (~46 requiring
  original authoring, ~60 h of learner time, the 110–150 h authoring estimate that D5's
  recommendation rests on) is anchored to the wrong one. The stub uses 61, which is what the
  tables actually contain, so the Training index reads "11 of 61 lessons outlined". Whether the
  four missing modules were dropped from a draft or the total was simply mis-added is not
  recoverable from the document; worth five minutes before anybody schedules P-06 against the
  hours estimate.
- **Nothing in this repo checks `scripts/` against the schema.** `harness-invariants.test.ts`
  walks `src/`, `db:verify` and the schema assertions walk `supabase/`, and the Gate does not
  execute the scripts at all — so `scripts/seed-demo-team.mjs` kept writing `tags: []` for a
  whole sprint after the column was dropped, and the only thing that found it was a person
  trying to seed a review team. Fixed for that instance. The general case is 15 scripts in
  `scripts/`, several of which write rows (`seed-demo-team`, `seed-review-states`, the six
  probes), and none of them is compiled, linted against the generated types, or run by CI. The
...
- **✅ FIXED 2026-08-29 (Sprint 34).** `season_event_id` on `scouting_reports`, a **composite** FK
  ...
  **Two halves, neither sufficient alone**: `eventsIn` groups by identity when present and by a
  ...
  (the exit criterion says "the current event's reports" and says nothing about the entity). The
  fix is a `season_event_id uuid REFERENCES competition_events(id)` on `scouting_reports` plus a
  picker that falls back to free text, which is a migration and a form change — small, and out of
  the criterion this sprint was measured against.
- **A pit-scouting `kind` column is NOT in the schema and was deliberately left out.** P-02's
  exit criterion permits "a migration for `kind`/`alliance`/`station`" and this sprint added the
  last two only: `kind` exists to separate match reports from PIT reports, and pit scouting is
  not in the minimal set. Adding a column whose only value is `'match'` would be a gate with no
  door (`docs/failure-modes.md` section 7, four sprints and seven instances). Recorded so the
  next reader knows the omission was a decision.

*From Sprint 25 — the sync engine, `v2/sprint-25-sync-engine` (2026-08-23):*
- **The realtime channel binds 36 `postgres_changes` subscriptions per tab, not the 24 the
  assessment measured.** 12 tables (11 `SYNCED_ENTITIES` plus `checklists`) × INSERT/UPDATE/
  DELETE; Sprint 18's four new entities are the difference. SYNC-04's connection half is fixed —
  a hidden tab releases its socket — but the per-message fan-out is untouched, and it was left
  untouched on purpose: dropping `seasons` would undo **B22** (season deletions reaching other
  devices is exactly what that bug was, and `REPLICA IDENTITY FULL` was added for it), and
  subscribing per route costs a WebSocket handshake per navigation on the worst connection in
  the product. The assessment's own estimate puts messages at 375k against a 2M quota, so this
  is not under pressure for beta. Revisit with the Pro-tier move (D7), where 500 connections and
...
- **✅ FIXED 2026-08-29 (Sprint 35).** `auth.updateEmail()` plus an "Email address" card in Edit
  ...
  first half — can only do it by asking Kevin, and there is no runbook entry for that either. The
  server side is now correct and complete (`sync_user_to_team_members` carries the change onto
  every child's roster row, verified through both the `public.users` and `auth.users` paths), so
  this is a missing screen rather than a missing capability: one input, `auth.updateUser({ email })`,
  and the confirmation round trip Supabase already sends. Worth doing before beta, because the
  one address a coach has for a managed child is the guardian's and it is currently unchangeable.

*From Sprint 22 — the September list, `v2/sprint-22-september-list` (2026-08-23):*
- **✅ FIXED 2026-08-24 (Sprint 29).** `CheckIn.tsx` calls `getMemberInitials(currentMember)`.
  The canonical helper was already imported into that file for the display name, so the copy was
  never even saving an import. Was: **A ninth display-initials implementation, and it disagrees with the canonical one.**
  `src/components/meetings/CheckIn.tsx:344-349` derives a member's initials inline —
  `getMemberDisplayName(m).split(/\s+/).map(p => p[0]).join('').slice(0, 2)` — while
  `src/lib/member-utils.ts` `getMemberInitials` takes the FIRST and LAST initials. For
  "Mary Jane Watson" the canonical answer is `MW` and CheckIn renders `MJ`. One call site, one
  line to fix, and it is the same class Sprint 1 deleted seven copies of: the check-in avatar and
  the roster avatar are two screens a coach can have open at once. `member-utils` is already
  imported in that file for the display name, so the fix is smaller than this entry.
- **Five test files inline-mock `offline-db` and omit `indexedDBStorage`, which silently strips
  the store's persistence.** `Dashboard.test.tsx`, `ParkedChangesDialog.test.tsx`,
  `dead-letter-reason-is-immediate.test.ts`, `entitlement-re-ask-on-expiry.test.ts` and
  `JoinTeam.test.tsx` each pass a `vi.mock(..., () => ({...}))` factory that does not export it,
  so `createJSONStorage(() => indexedDBStorage)` is `undefined` and zustand's persist middleware
  degrades to a plain store with **no `.persist` at all**. Anything reading `store.persist`
  throws; in Sprint 22 that took the whole dashboard route into its error boundary. The hook was
  hardened (`persist?.hasHydrated() ?? true`) but the mocks still lie about the module.
  `mock-drift.test.ts` cannot see this: it compares `src/lib/__mocks__/*` against the real
  modules and knows nothing about inline factories. Either extend it to parse inline `vi.mock`
  factories, or convert the five to `vi.mock('@/lib/offline-db')` plus the manual mock.
- **`managed_profiles.notes` has no length limit, in the column or at the input.** WALK-B-10
  capped `full_name` at 120 in both; the `notes` textarea directly beneath it in
  `AddChildDialog.tsx` has no `maxLength` and the column has no CHECK. It is guardian-authored
  free text about a child (allergies, pickup arrangements), it is rendered in `GuardianView`, and
  `NOTES_MAX_LENGTH = 500` already exists in `scouting-validation.ts` as the number this kind of
  field uses. Lower severity than the name because SEC-05 keeps it private to the guardian, but
  it is the same gap in the same dialog.
- **Nine member-name render sites have no overflow protection**, found while checking WALK-B-10's
  rendering half. A managed child's name reaches all of them, because
  `join_team_with_managed_profile` copies the child's `full_name` into the `team_members` row.
  `SubTeamManager.tsx:141` (read-only member chip: no `truncate`, no `min-w-0`),
  `PreMatchChecklist.tsx:215` (assignment chip marked `shrink-0`, so it squeezes the item text
  rather than giving way), `SprintList.tsx:45`, `SprintArchived.tsx:76`, `SprintCalendar.tsx:45`,
  `SprintTaskActivity.tsx:140`, `AttendanceRoster.tsx:370`, `AttendanceSummary.tsx:199`,
  `AdminTransferPanel.tsx:235`. Bounded at 120 characters by the column now, so this is a layout
  question rather than a runaway one — but 120 unbroken characters in a `shrink-0` chip is still
  a squeezed row. `ConfirmDialog` and the four guardian paragraphs were fixed in Sprint 22
  because that is where a child's name lands; these nine were left.
- **`@supabase/supabase-js` cannot go past 2.89 without changing the sync engine's typing.**
  2.112 (which closes the `ws` advisory) parses the `select()` string at the type level, and the
  one data path builds that string at runtime from the entity registry, so the row type resolves
  to `never` and six `tsc` errors appear: `server-pull.ts:466,471,478,482` and `sync.ts:560,571`,
  all `Argument of type '"<column>"' is not assignable to parameter of type 'never'`, plus one in
  `erasure.db.test.ts:309`. The devDependency is pinned at `2.89.0` exactly so `npm install`
  cannot take it by accident. The upgrade needs its own change with the B1–B26 regression suite as
  its evidence (principle 2), and it should be taken before the supabase-js line drifts far
  enough that the jump is larger. The advisory it closes does not reach the browser.
- **The encrypted nightly artifact has never been decrypted outside the runner.**
  `scripts/restore-rehearsal.mjs` proves the gpg round trip with a throwaway passphrase and
  proves a hosted dump restores completely (65/65 rows, 2026-08-23), but nothing has yet proved
  that `BACKUP_PASSPHRASE` opens the `.gpg` GitHub is holding. `gh run download <id>` plus one
  `gpg --decrypt` is fifteen minutes, and the moment to find out is not during an incident.
- **The `seed:review` teams contain no work at all.** Five teams, **0 tasks, 0 scouting reports,
  0 match plans** between them (`select count(*) from tasks` → 0). Any dashboard, board or
  progress check written against that seed compares zero to zero — the vacuous-check class
  `HANDOFF_BUILD.md` warns about, and it bit within ten minutes of running the app in Sprint 22:
  the "Welcome back" half of WALK-B-06 could not be verified against the seed at all and needed a
  task created through the board first. `scripts/seed-demo-team.mjs` builds a populated team and
  is not what `seed:review` runs. Giving Iron Falcons a handful of tasks, one scouting report and
  one match plan would make the review seed exercise the screens it exists to review.
- **The `session_replication_role` restore trap does not reproduce, and the discrepancy is
  unexplained.** `docs/beta-ops.md` measured 32 teams restored and zero members on 2026-08-23;
  the same day, the same document's procedure with the trigger blocks REMOVED restored all 65
  rows with zero errors. The mechanism for the success is nailed down (`supautils`
  allow-lists `session_replication_role` for the privileged role), but not the mechanism for the
  earlier failure — which means one of the two measurements was taken against a configuration
  nobody has written down. Worth ten minutes with `git log` on `beta-ops.md` and the CLI version
  that was installed then, because the runbook's central warning currently rests on it.

*From the 2026-08-23 production release:*
- **Every `catch` in the operator console discards the database's own explanation.** A
  supabase-js error is a `PostgrestError` — a plain `{message, code, details, hint}` object,
  never an `Error` instance — and the handlers all read
  `err instanceof Error ? err.message : '<generic>'`. So a refused or failed action shows
  "Could not erase this person" whatever actually went wrong, including the cases where the
  database said something useful. **Seven handlers in `OperatorConsole.tsx`**, plus
  `AcceptAdminNomination.tsx` (2), `AdminTransferPanel.tsx` (2), `sync.ts` (2),
  `offline-db.ts` (1), `error-reporting.ts` (1). The fix is one shared
  `messageFromUnknownError(err)` helper rather than a line in each, which is why it was not
  done under a release verification. `OperatorConsole.test.tsx` pins the current behaviour
  and says in a comment that it documents a defect rather than an intention.
- **The documented restore has never been performed, and it does not work as written in a
  plain database.** Restoring the dump into a bare `CREATE DATABASE` produced 29 errors;
  into one carrying Supabase's `auth`/`extensions`/`vault` schemas it produced 5, all from
  a deliberately simplified stub. Two of the original 29 matter and are worth knowing
  before a real restore: `SET session_replication_role = replica` was **denied** to the
  local `postgres` role, so the data half loaded with triggers live, and SEC-01's
  membership trigger then **rejected rows** — silently, since the CLI does not stop on
  error. A restore that drops the admin's `team_members` row while reporting success is a
  worse outcome than one that fails. `beta-ops.md`'s check (`grep -c '^INSERT INTO '`)
  proves a dump CONTAINS data; nothing yet proves one RESTORES.

*From Sprint 21 — SEC-11, `v2/sprint-21-erasure` (2026-08-23):*
- **SEC-11's other four items are not done and were never in this sprint's definition of
  done: a team JSON export for the admin, invoices, seat reduction, and team rename/number
  editing.** The export is the one with a promise attached, and the promise is weaker than it
  looks: the Privacy Policy says *"You can ask us for a copy of what we hold about you … we
  do not require a formal process — just ask"*, which a manual answer satisfies exactly the
  way the erasure runbook used to. Erasure was the half promised as something that simply
  happens, so it went first. Team rename is the smallest of the four: `teams_update_manager`
  already exists as a policy and there is no `.from('teams').update` anywhere in `src/`
  outside the nomination columns — a UI-only change against a rule that is already written.
- **An operator who has performed any audited action can never have their `auth.users` row
  deleted**, because `operator_actions_operator_user_id_fkey` is NO ACTION and an audit entry
  must be able to name who made it. That is correct and should stay, but it has one real
  consequence worth knowing before somebody "fixes" it: `npm run seed:review` cannot be
  re-run after any operator action without `npm run db:reset` first. The seed now fails with
  that sentence instead of a misleading "already registered". Note that 
  `operator_erase_user` handles an operator correctly — it anonymises rather than deletes,
  which is exactly why that design was chosen.
- **A unit test timed out once in six full runs and it is not understood.** `Dashboard > hands
  the board only the current season's tasks` failed at **14 759 ms** on the run immediately
  following a database reset, a re-seed and a browser probe; it passed in isolation and in
  five consecutive full runs afterwards. No assertion failed — it ran out of time. Recorded
  rather than dismissed because "it passed on the retry" is how this repo's one genuinely
  intermittent e2e failure went unexplained for three sprints.

*From Sprint 19 — the accessibility set, `v2/sprint-19-accessibility` (2026-08-23):*
- **✅ FIXED 2026-08-24 (Sprint 29), by the second of the two candidate fixes this entry names —
  the one that also breaks the cascade.** `globalSetup` sweeps `team_number ~ '^3[0-9]{4}$'` before
  the first file. Ranges verified disjoint first, because a sweep that took out a developer's own
  data would be worse than the problem: the review seed uses four-digit numbers plus 12345, the
  demo seed 14822, and the e2e pack draws from 50000–95000.
  **Proved both ways against a real database.** Three orphans planted at 30001–30003 — the exact
  numbers `fixtures.ts` hands out first — reproduced the failure with the sweep disabled
  (`duplicate key value violates unique constraint "teams_program_number_unique"`, **20 tests
  skipped**), and with it enabled the same file ran **20 passed** and left zero rows behind.
  **AND THE FIRST VERSION OF THE SWEEP WALKED STRAIGHT INTO THIS DOCUMENT'S OWN TRAP.** Built with
  `shell: true` on Windows and the pattern in an ARGUMENT, cmd ate the `^` as its escape character
  and psql answered `syntax error at or near "DELETE"`. The statement goes on stdin via `-f -`
  now, which is how `db:assert` has always done it. It surfaced immediately only because the sweep
  was written to fail loudly rather than to swallow its own error — a silent version would have
  handed the run back to the duplicate-key cascade with one more layer between symptom and cause.
  Was: **An interrupted `test:db` run poisons the next one, with an error about the fixture rather
  than about the test — the exact outcome the fixture numbering was written to prevent.**
  `nextFixtureTeamNumber()` is a per-PROCESS counter starting at 30000 (`src/test/db/
  fixtures.ts:132`), and `Fixtures.cleanup()` only runs if the process reaches it. Kill a run
  part-way — Ctrl-C, a CI timeout, an OOM — and its teams survive at 30001, 30002…, so the
  next run's very first `createTeam` fails `duplicate key value violates unique constraint
  "teams_program_number_unique"` and takes **20 suites** down with it. Observed twice while
  closing Sprint 19, and it cascades: the collided run also dies before cleanup, so the state
  is self-sustaining until someone runs `db:verify` or deletes the rows by hand
  (`delete from teams where team_number ~ '^3[0-9]{4}$'`). The fixture comment says the
  numbering exists so a shared number cannot "take the whole of `tenant-isolation` down with
  an error about the fixture" — which is precisely what leftovers now do. Two candidate
  fixes, neither taken because this is outside Sprint 19's IDs: seed the counter from a
  per-run random base (the e2e pack already had to learn this in Sprint 17), or have
  `globalSetup` sweep `team_number ~ '^3[0-9]{4}$'` before the first file. The second also
  fixes the cascade.
- **`npm run gate:db` is being killed part-way through `test:db` on this machine, and it is
  NOT a Sprint 19 regression — it reproduces on `1610c0f`, the commit before the sprint.**
  Exit 127, at a different suite each time, never on a failing test; the same command run
  directly as `npx vitest run --config vitest.config.db.ts` completed **626 passed, exit 0**.
  Both shells (Git Bash and PowerShell) behave the same, containers stay healthy and ~4.5 GB
  of RAM stays free, so it is not obviously memory. The one lead is the `DEP0190` warning
  printed immediately before each death — something is spawned with `shell: true`, and 127 is
  literally "command not found"; `src/test/db/globalSetup.ts:34` is the only such spawn in
  the repo (`execFileSync(..., { shell: process.platform === 'win32' })`). Worth chasing
...
- **✅ FIXED 2026-08-24 (Sprint 29).** `/onboarding` and `/create-team` use `<SignInFirst />`,
  the component this repo had already written for exactly this and then not used on these two
  routes. Was: **`registration.spec.ts`'s intermittent failure is no longer unexplained, and the fix is one
...
- **✅ FIXED 2026-08-24 (Sprint 29), together with the Sprint 11 entry below that describes the
  same line.** `offline-db.ts` has a `describeSyncError()` that reads `.message` off the plain
  PostgREST object and appends the SQLSTATE `code`, because the message is often the generic half.
  Five regression tests in `offline-db.test.ts`, three of which were watched failing with the
  literal `expected '[object Object]' not to be '[object Object]'`. **The one that stayed GREEN
  against the defect is the interesting one** — a test throwing `new Error('boom')` passes either
  way, because an `Error` is the single input the broken branch handled correctly, which is why
  this survived unnoticed. Was: **`SyncFailure.lastError` stores `"[object Object]"` for a PostgREST refusal.** Read straight
...
(2026-08-23):*
- **✅ RESOLVED — verified against GitHub 2026-08-24 in the triage pass.** `gh workflow list --all`
  now returns "Nightly encrypted database backup" as active, and `gh run list` shows a **scheduled
  run succeeding at 2026-08-24T08:02:30Z** as well as the manual `32670045597`. The 2026-08-23
  production release pushed `main`, which is the one thing this entry said was missing — the row in
  the table above records it, but this entry was never struck through, so it still read as the
  blocker it had stopped being. Was: **The two backup secrets, and an uptime signup — both Kevin's, both blocking a finished
...
- **✅ DONE 2026-08-29 — REHEARSED, AND IT WORKED. The first proven recovery in this project's
  ...
  **It is marked UNREHEARSED in its own first paragraph**, derived from the local rehearsal and from
  ...
  Supabase project. The scratch-database rehearsal in the release row does not speak for what a
  fresh hosted project provides. Was: **The last mile of the restore is still unrehearsed: a HOSTED artifact into a NEW Supabase
...
- **✅ FIXED 2026-08-24 (Sprint 29) — see the Sprint 16 entry above; these two entries were the
  same defect described from two angles, and are now one.** Was: **A dead-lettered change records `"[object Object]"` as its `lastError`, for every server
...
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** `scripts/restore-rehearsal.mjs:50`
  imports it, which is this entry's own suggested remedy ("use it for raw SQL in the db suite").
  Was: **`pg` is an unused devDependency.** `package.json:73` carries `"pg": "^8.20.0"`; after Sprint
...
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** The four PNGs are no longer
  byte-identical and no longer 784 KB (`falcon_logo` 10 KB, `icon-192` 7 KB, `icon-512` 23 KB,
  distinct md5s; `logo.png` is gone), and today's build reports a **1,582 KiB** precache against
  the 5.01 MB below. Closed by Sprint 22's `perf(pwa)` commit; the entry was never struck through,
  and it was still carrying a 🔴. Was: **The precache is 60% one image, copied four times.** `logo.png`, `falcon_logo.png`,
...
- **✅ FIXED 2026-08-24 (Sprint 29).** `signUpWithEmail` passes `emailRedirectTo:
  authRedirectUrl()`, so every auth flow now goes through the one helper that exists precisely so
  no flow grows its own answer. Was: **`signUpWithEmail` passes no `emailRedirectTo`.** So the signup confirmation link is built
...
- **✅ PARTLY RESOLVED — verified 2026-08-24 in the triage pass.** `public/404.html` exists, so
  the Pages SPA fallback this entry asks for is in place. The now-dead `/auth/callback` route is
  the half that remains. Was: **No `404.html`, so every non-hash deep link 404s on GitHub Pages**, and **the now-dead
...
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** The file no longer exists;
  `supabase/tests/` holds only `schema_assertions.sql`. The entry's "wire it in or delete it"
  was answered by deleting it. Was: **`supabase/tests/preflight_security_audit.sql` (104 lines) is orphaned** — referenced by no
...
- **✅ DELETED 2026-08-24 (Sprint 29), on this entry's own terms.** It said "if a fourth sprint
  passes with it still empty, delete it"; twenty-two did, and it had exactly one occurrence in
  `src/` — its own declaration. Was: **`MEMBER_REQUIRED_ATTESTATIONS` is now an empty named constant with a documented reason.**
...
*From Sprint 5:*
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** The `setTimeout(0)` deferral this
  entry describes as "left uncommitted in the working tree for review" is committed:
  `src/lib/auth.tsx:251`, with the Web Lock reasoning preserved in the comment. It was still
  carrying a 🔴. Was: **A restored session can drop the user into the forced age-profile screen.**
...
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** No test file matches `orange-`; the
  rename this entry deferred has happened. Was: **`orange-*` is now `forge-*` in app source, but the tests still say `orange-`.** The
...
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** 30 references in `src/` outside
  tests; the Sprint 6 admin console this entry was waiting for exists. Was: **`transfer_team_admin` has no caller.** Written because the one-admin unique index makes
...
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** `include` is now
  `["src", "*.ts", "*.tsx"]`. Was: **`tsconfig.json` includes `components` and `services`** — root-level directories that no
...
- **✅ RESOLVED — verified 2026-08-24 in the triage pass.** Neither `.calendar-grid` nor
  `.transition-smooth` appears in `src/index.css`. Was: **Unused CSS survives:** `.calendar-grid` and `.transition-smooth` in `index.css` have no
